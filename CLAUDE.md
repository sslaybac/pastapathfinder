...
- **`specs/`** — documents that govern *building* the product. Agents consult these; Do not modify files under specs/ on your own initiative. Spec files may be created or edited only when the user's prompt for this session explicitly directs it (e.g., producing specs/tasks.md, checking off a completed task, applying an approved amendment). Treat any other spec change as out of scope and flag it instead.
  ...
  - `specs/design.md` — HOW the system is built. **APPROVED 2026-07-20; amended 2026-07-21, -22, -23 and -24.** 2026-07-21: D16–D20, resolving the step-4 task-breakdown clarifications (the `module` node kind and call-site attachment rule; unit-explicit coverage counts; wholesale entry-point recomputation and the metadata change gate; derived class/module reachability; the index-only viewer), plus D21 extending `--debug` to every subcommand, raised while implementing task 1.1. Since then, each raised while implementing the task named: D22, node-ID module segments are path segments (task 2.2); D23, which side of the analyzed-set boundary names a constructor call (task 2.3); §3.5's import table built with stdlib `ast` (task 2.4); D19's two clarifications and §3.9's exclusion of external leaves from `dead_code()` (task 3.4); §8-O7 recorded as a live defect (task 3.4); D24, the §3.4 adapter protocol's `prior_nodes` and `cache_fallback` (drift from task 4.1, recorded during task 4.4). Source of truth for architecture, component interfaces, schemas, and file formats; its §2 decision log records every technology choice with its evidence.
  ...
  - `specs/tasks.md` — the ordered, verifiable implementation tasks. **APPROVED 2026-07-21.** Source of truth for build sequencing: 23 tasks across 5 milestones, each with its deliverable, the FR/AC and design.md sections it implements, its dependencies, and the verification that proves it done. Implementing agents work one task at a time; §5 records the five task-level ambiguities found during derivation and the design decisions (D16–D20) that resolved them.
...
- [x] Step 3 — Technical design (`specs/design.md`, approved 2026-07-20; amended 2026-07-21 with D16–D21, and subsequently with D22–D24 and the section-level amendments listed in the repository map above, each raised during the implementation task that hit it). Engine evaluation completed via six prototype sessions (evidence: `prototypes/engine-eval/`); OQ-1/OQ-7 resolved (mypy 2.3.0 / Python); OQ-2 and OQ-4 deliberately remain open until the viewer milestone.
- [x] Step 4 — Task breakdown (`specs/tasks.md`, approved 2026-07-21). 23 tasks in 5 milestones, pipeline before viewer (requirements §7); §8-O5's benchmark pins copied through to the task verification criteria, §8-O6's WSL pass scheduled as task 5.4. Five derivation-time ambiguities were resolved into design.md as D16–D20 rather than guessed.
- [ ] Step 5 — Iterative execution: work `specs/tasks.md` in order, one task per session. Task 1.1 is the entry point. Check off each task in `specs/tasks.md` as it completes (this is an explicitly permitted spec edit under rule 2). Extra human review is recommended at the six checkpoints in tasks.md §4.
