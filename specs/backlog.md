...
- **Notes:** Fits the tech-debt survey workflow (US-2) naturally. The per-site unresolved diagnostics (AC-14.2, with callee text per design.md §4.3) are a second data source this report should draw on — unresolved sites are the *strongest* dynamism signal, stronger than resolved-but-ambiguous edges.
...
- **Depends on:** OQ-1 — the selected engine must have an error-recovering parser. Requires propagating the third status through FR-7's reconciliation, the index, and all presentations. *Note 2026-07-20:* OQ-1 resolved to mypy, whose parser is **not** error-recovering — a syntax error fails the whole file, which is exactly v1's skip behavior. Implementing this entry therefore requires a supplementary parsing path for broken files (not an engine setting), and the "error-recovering parser" tiebreaker recorded here played no role in the final selection.
- **V1 insurance:** *Disposition final 2026-07-15: SA-8 deferred; this entry is its permanent home.* Insurance inventory from the deferral review: (1) report format-version field — AC-42.4, added as this feature's specific accommodation, so the fourth file status is a versioned format change; (2) index-side needs are already covered by existing generic mechanisms — the file-status attribute rides FR-21's attribute fields, unresolved-region spans reuse FR-37's span vocabulary, per-file keying (FR-24) is unaffected, and the index schema change is an FR-39 version bump; (3) FR-43's exit-code semantics should be *designed* (design-stage note) in terms of "fully analyzed vs. not" so a fourth status maps without recoding — *honored: design.md §3.1 computes exit codes exactly that way*; (4) "has an error-recovering parser" is a recorded OQ-1 tiebreaker.
...
- **V1 insurance:** Extensive and deliberate — FR-21 (generic schema), FR-22 (namespaced IDs), FR-23 (adapter boundary), FR-8 (pluggable detectors), FR-2 (per-language convention sets), plus the §6 anti-requirement barring a speculative plugin framework. The adapter interface is now concrete: design.md §3.4's `LanguageAdapter` protocol.
...
- **Depends on:** FR-8's detector structure (pure addition). Two design problems to interrogate before building: (1) *precision posture* — the heuristic ("module-level executable statements beyond imports/definitions," or narrower, "CLI-parser construction at module scope") over-fires on modules with import-time side effects (a `config.py` building dicts at top level is not an entry point); over-firing is the safe direction for dead-code findings (spurious entry points suppress false "dead" results, never create them) but pollutes the entry-point list, so the detector needs its own stated posture the way FR-14 states one for edges. (2) *Slice-target semantics* — the emitted node represents module top-level code, not a function; whether module-level flow is sliceable is exactly the AC-17.2 boundary, so the detector's node type/attributes must be designed together with that answer. *Note 2026-07-20:* design.md resolved the slice-target half in v1's favor — every file gets a sliceable module-body node (`python_role="module_body"`, design.md §3.5/§4.1) as FR-9's target, so this detector's emitted entry node has a ready-made target and problem (2) reduces to reusing it. Problem (1), the precision posture, remains the open interrogation.
...
## B-22 — Narrowed method-dispatch over-approximation layer

- **What:** A syntactic resolution layer that generates candidate `calls` edges for attribute-call sites the mypy backbone leaves unresolved: for `x.m(...)`, edges to classes defining `m`, narrowed by (N2) self/super class-hierarchy restriction and optionally (N1) receiver attribute-profile intersection (duck typing); every emitted edge FR-40-flagged ambiguous. Fires only on mypy-unresolved sites — restriction to that subset, not narrowing, is what preserved union precision in the evaluation.
- **Origin:** The FR-14 fitness gap identified during technical design (2026-07-18, recorded as F-3 in the design discussion): the selected precise engine emits nothing where it cannot infer, while FR-14's rationale wants candidates. Evaluated empirically in prototype sessions 4–5 (`prototypes/engine-eval/FINDINGS-namematch.md`; `FINDINGS-session5.md` Part 3); deferred by stakeholder decision 2026-07-18 under the C-6 discipline ("budget it as a feature, not a patch"; portfolio tiebreaker). The requirements trace is C-11.
- **Depends on:** v1 shipped. A bounding posture for the surviving fanout tail — N2+N1 narrowing still leaves attribute-chain-receiver constructor calls at worst-case fanout (max 760 measured); the evaluation's threshold ("p99 in the low tens") was met overall but the tail needs a stated posture, FR-14-style. Extension of the §6 item 10 amendment — the bracketed method-dispatch clause was drafted 2026-07-18 and deliberately not adopted; adopting this entry requires that requirements amendment. An incremental design of its own: the layer's re-computation cost is bounded by churn in common method names, not changed-file count (FINDINGS-namematch §4), so it does not ride FR-24's file-keyed model unmodified. An FR-39 schema-version bump if any new attribute vocabulary is needed (expected: none — `is_ambiguous` plus `attrs` suffice).
- **V1 insurance:** Substantial and deliberate — FR-40 (edges arrive pre-flagged); the AC-14.2 per-site unresolved diagnostics with callee text (design.md §4.3) are the exact audit trail identifying which sites this layer would cover; D15's import table already lives in the adapter as reusable infrastructure; C-11 documents the gap this closes; the full evaluation evidence and the probe implementation survive in `prototypes/engine-eval/`.
- **Notes:** Key measured numbers to re-verify at interrogation time: covers ~54 % of mypy-unresolved sites (the other 46.1 % is bare-name value-flow, permanently invisible to any syntactic layer — that share belongs to B-1/B-2's territory); union effect +2.2 pts recall / −2.0 pts precision when restricted to unresolved sites; N2 is load-bearing (kills the `super().__init__` torture cases), N1 marginal; payoff is codebase-dependent (Django 6.4× fanout reduction, pandas 1.26×) — consider a per-run toggle. Not a substitute for value-flow analysis, and should not be pitched as one.

## B-23 — Fine-grained incremental daemon (mypy dmypy machinery)

- **What:** A long-lived analysis process built on mypy's fine-grained update machinery (`mypy.server.update.FineGrainedBuildManager`, the dmypy internals) for sub-second re-analysis of small edits — below the v1 batch path's measured 13.2 s (core change) / 2.7 s (leaf change).
- **Origin:** FINDINGS-mypy Q3's optional variant, dropped in session 4 when the batch path passed FR-30 with 2.3× headroom; recorded at design (design.md D6 discussion) as not-needed-for-v1.
- **Depends on:** A demand signal — evidence that 13 s is ever actually too slow in a live session (the FR-30 threshold was set as the stakeholder's tolerance; this entry only matters if practice disagrees). Deepens design risk R1: the fine-grained machinery is deeper into mypy's semi-public internals than `build()`, so the D1a pin-and-revalidate tax grows. A daemon-process posture consistent with FR-33/FR-34 (local, unprivileged — fine in principle; lifecycle management is the new work). Interacts with the B-19/B-21 viewer-control gradient if the daemon is ever viewer-triggered.
- **V1 insurance:** None needed — the FR-23 adapter boundary localizes the change to `adapters.python`, and D6's evict-and-merge discipline (proven equivalence-preserving) is reusable as the merge layer regardless of what feeds it.
- **Notes:** Prime exit-rule candidate: if v1 latency proves fine in practice for a release or two, delete this entry with a dated note rather than letting it linger.
