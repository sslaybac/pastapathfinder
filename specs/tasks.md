...
- [x] **Task 2.4 — `externals.py`: external leaf nodes (D15's two sources)**
  - **Deliverable:** External-node emission from (a) mypy resolving a call to a fullname outside the analyzed set, and (b) the adapter's per-module import table supplying the qualified name where mypy left the site unresolved but the callee is a name imported from an unanalyzed module; nodes deduplicated by qualified name, `is_external=1`, no span, no outgoing edges (enforced). The import table is built with **stdlib `ast`** over the re-extracted set, never from the engine's trees — requirements §6 item 10 exempts this mechanism only on those terms (amended 2026-07-22; rationale and cost on design.md §3.5). It has one consumer, `externals.py` itself: D18 gives the Django URLconf detector its own stdlib-AST table, so task 3.3 does not consume this one.
...
- [x] **Task 4.1 — `incremental.py`: hash gate, evict-and-merge, fallback, re-analysis report**  ⚠ *highest-risk*
  ...
  - **Handoff from task 3.4 (recorded 2026-07-23):** the detector pass, reachability, and the dead-code findings now run inside `runner`'s `full_write` block, in that order (D18's recompute rule, then D19). This task moves that sequence into the merge order §3.6 specifies — after the external-node cleanup, before reachability — rather than adding it. Know before you start that D18's wholesale re-emission walks into **design.md §8-O7**: two entries declared for one target on one source line collide on their §4.1 `entry:` ID and the store rejects the pair, failing the run. It is a live defect on legal Django input, it is unfixed, and it is a grammar decision rather than something to work around inside the merge.
  - **References:** design.md §3.6, D6, D18, §5.3, §8-O7; requirements FR-24 (incl. C-9's transitive-closure floor), FR-30, FR-35, EC-7, EC-13 (pipeline half).
...
- [x] **Task 5.2 — `viewer/static/`: the no-build frontend**  ⚠ *highest-risk*
  ...
  - **Verification:**
    ...
    - FR-33: **no CDN references, no npm toolchain** — a test greps the shipped assets for external URLs and fails on any; the page renders with all external network access blocked. *Amended 2026-07-24, stakeholder-approved in session, raised while implementing this task:* the grep as worded is unsatisfiable alongside D8's mandatory vendoring — `cytoscape.min.js` carries MIT license URLs and the dagre bundle carries ECMAScript citations, both inside comments, in files that must ship byte-identical to what was fetched. The check is therefore split at the line where it means something, and both halves are required: (a) the **authored** assets (`index.html`, `app.js`, `style.css`) contain no external URL of any kind — the strict grep, unchanged, on the files that could actually regress; (b) across **all** shipped assets, vendored included, nothing a browser fetches may point off the machine — every declared subresource resolves to a local route served by this server, no `@import`/`url()`, every source map relative, and no CDN host named anywhere. "Renders with external access blocked" is proven at runtime rather than by inspection: the page is driven through every workflow with `fetch` recorded and `XMLHttpRequest`/`WebSocket`/`EventSource`/`Image`/`sendBeacon`/`importScripts` wired to record-and-throw, and each request must be a same-origin `/api/…` call. This is a wording fix to this bullet only; D8's "no CDN, no npm" is enforced exactly as design.md states it, and no design.md change is owed.
...
- [x] **Task 5.3 — `docs/`: the owed user-facing set**
